    ...
    <div className="relative w-56 p-2 bg-white shadow-md rounded-lg overflow-hidden flex flex-col justify-between h-full">
      ...
      <div className="flex items-end justify-evenly">
        <div className="flex flex-col leading-tight">
          <p className="text-[15px] text-gray-600 my-1">Cost:</p>
          {item.discount > 0 ? (
            <>
              <p className="text-gray-400 line-through text-[16px] my-1">
                {item.cost}$
              </p>
              <p className="text-red-500 font-bold text-[20px] my-1">
                {item.cost - item.discount}$
              </p>
            </>
          ) : (
            <p className="font-bold text-[20px] my-1">{item.cost}$</p>
          )}
        </div>
